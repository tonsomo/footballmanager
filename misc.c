...
void MISCSelectTeam(GAME *g)
{
    ...
    while (Found == 0)
    {
        ...
        #else
        IOText(-1,158,COL_GREEN,COL_BLACK,"Press R for more teams");
        #endif
        ...
        for (n = 0;n < g->Division[Div].NoTeams;n++)
        {
            ...
            #ifdef GP2X
            if(choice==n)
                 IOText(x+24,y,COL_BLACK,COL_GREEN,g->Division[Div].Teams[n].Name);
            else
...
void MISCTitle(void)
{
    ...
    IOText(-1,170,COL_GREEN,COL_BLACK,"GP2X Port By Jonn Blanchard");
    INITPressEnter();
}
...
char *MISCGetFileName(char Transfer)
{
    ...
    IOText(5,108,COL_CYAN,COL_BLACK,"Select a slot");
    while(!finish) {
        for(i=0;i<10;i++) 
            if(n==i)
                IOChar(120+(i*8),108,COL_BLACK,COL_CYAN,i+48);
            else
                IOChar(120+(i*8),108,COL_CYAN,COL_BLACK,i+48);
