...
int HWJoy() {
    ...
    if(SDL_JoystickGetButton(joystick,VK_VOL_UP)!=0) {
        volume++;
        if(volume>100) volume=100;
        gp2x_sound_volume(volume,volume);
    }
    if(SDL_JoystickGetButton(joystick,VK_VOL_DOWN)!=0) {
        volume--;
        if(volume<0) volume=0;
        gp2x_sound_volume(volume,volume);
    }
    
    for(i=VK_UP;i<=VK_TAT;i++)
...
void HWInitialise(void)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO|SDL_INIT_JOYSTICK)   /* Try to start up SDL */
                                        == -1) MERROR(); 
    #ifndef GP2X
